...
def do():
    ...
    # 전체 주차 문제 리스트 불러 오기
    with open(path.join(path.abspath(path.dirname(__file__)), 'problems.json'), 'r', encoding='utf-8') as f:
        problems = json.load(f)
    f.close()

    # 최신 날짜 불러 오기
    with open(path.join(path.abspath(path.dirname(__file__)), 'date.json'), 'r', encoding='utf-8') as f:
        ls = json.load(f)
    f.close()
    ...
    for user in users:
        ...
        if cnt == len(problems[len(ls) - 1]):
            user['atnd'] = True
    with open(path.join(path.abspath(path.dirname(__file__)), f'check{len(ls)}.json'), 'w', encoding='utf-8') as chk:
